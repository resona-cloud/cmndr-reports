...
function scoringEngine(questionnaireAnswers, firecrawlSignals, pagespeedSignals) {
  ...
  // Revenue Exposure (25%)
  const revenueRaw = avg([q.q1, q.q2, q.q3, fc.has_clear_headline, fc.has_primary_cta, fc.cta_count]);
  const revenueScore = Math.round(Math.min(100, revenueRaw * 20));

  // Conversion Friction (25%) — higher = less friction = better
  const conversionRaw = avg([q.q4, q.q5, ps.mobile_performance, ps.mobile_fcp, ps.mobile_cls]);
  const conversionScore = Math.round(Math.min(100, conversionRaw * 20));

  // Trust Surface (20%)
  const trustRaw = avg([q.q6, q.q7, q.q8, fc.has_testimonials, fc.has_trust_signals, fc.has_contact_info]);
  const trustScore = Math.round(Math.min(100, trustRaw * 20));

  // Market Visibility (15%)
  const visibilityRaw = avg([q.q9, q.q10, fc.service_page_depth, fc.content_length_signal]);
  const visibilityScore = Math.round(Math.min(100, visibilityRaw * 20));

  // Growth Readiness (15%)
  const growthRaw = avg([q.q11, q.q12, ps.desktop_performance]);
  ...
  return { overall_score: overallScore, overall_band: getBand(overallScore), pillars, severity_map };
}
